...
async def _call_create_guild(adapter: DummyAdapter, bot: SimpleNamespace) -> None:
    await adapter._api_create_guild(bot=bot, name="test-guild")  # noqa: SLF001


async def _call_delete_guild(adapter: DummyAdapter, bot: SimpleNamespace) -> None:
    await adapter._api_delete_guild(bot=bot, guild_id=1)  # noqa: SLF001


async def _call_modify_guild_mfa(adapter: DummyAdapter, bot: SimpleNamespace) -> None:
...
async def _call_create_guild_from_template(
    adapter: DummyAdapter, bot: SimpleNamespace
) -> None:
...
EndpointCall = Callable[[DummyAdapter, SimpleNamespace], Awaitable[None]]
